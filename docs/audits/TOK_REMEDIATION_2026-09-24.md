...
Les correctifs sont proposés sur une branche. Aucune migration, aucun déploiement Edge/production, aucune impression, aucun paiement et aucun merge vers main n'ont été effectués. Le statut de publication et les contrôles distants sont indiqués dans la PR. Ce lot ne constitue pas une certification complète de l'application.
...
La migration `20260924040000_restrict_internal_print_rpc_execution.sql` retire explicitement EXECUTE à PUBLIC, anon et authenticated sur six signatures exactes, et préserve service_role. Elle ne modifie pas les corps des fonctions, les RPC publics ni les privilèges par défaut. Une postcondition intégrée annule la migration si les droits effectifs ne correspondent pas au résultat attendu.
...
La migration `20260924040200_align_restaurant_public_read_policies.sql` aligne les deux chemins permissifs trop larges sur les critères de publication déjà présents dans restaurants_public_select. Elle ne transforme pas ces policies en RESTRICTIVE, afin de ne pas interdire les brouillons au propriétaire ou à l'administration. L'exception de restaurant de démonstration associé est conservée.
...
Contrôles non exécutés : pnpm lint, pnpm typecheck, pnpm test intégral, pnpm build, SQL PostgreSQL, navigateur/mobile, paiements réels et déploiement. Le checkout/dépendances et PostgreSQL ne sont pas disponibles localement. Aucune validation non exécutée n'est déclarée réussie. La relecture du diff a été effectuée sans reviewer indépendant.
...
3. Appliquer les trois nouvelles migrations dans l'ordre, sans rejouer ou réécrire les anciennes. Vérifier leur idempotence dans cette base.
...
3. supabase/migrations/20260924040000_restrict_internal_print_rpc_execution.sql
4. supabase/migrations/20260924040100_qualify_invoice_logo_storage_paths.sql
5. supabase/migrations/20260924040200_align_restaurant_public_read_policies.sql
