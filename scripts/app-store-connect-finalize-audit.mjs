...
console.log("BUILD", JSON.stringify(build?.data ? { id: build.data.id, ...attrs(build.data) } : null));
...
const appInfo = infos.data.find((row) => ["PREPARE_FOR_SUBMISSION", "READY_FOR_SALE", "READY_FOR_DISTRIBUTION"].includes(row.attributes?.appStoreState)) || infos.data[0];
...
const price = await request(`/v1/apps/${app.id}/appPriceSchedule?include=baseTerritory`, { optional404: true });
console.log("PRICE_SCHEDULE", JSON.stringify(price?.data ? { id: price.data.id, ...attrs(price.data), included: (price.included || []).map((row) => ({ type: row.type, id: row.id, ...attrs(row) })) } : null));
