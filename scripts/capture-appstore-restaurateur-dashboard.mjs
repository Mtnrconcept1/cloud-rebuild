...
try {
  ...
  const manifest = {
    ...
    temporary_restaurant_deleted_after_capture: true,
    temporary_subscription_provider: "none",
    temporary_subscription_plan: "elite",
    page_count: SCREENS.length,
    device_count: DEVICES.length,
    screenshot_count: SCREENS.length * DEVICES.length,
    screens: SCREENS,
    devices: [],
  };
  ...
  for (const device of DEVICES) {
    ...
    for (const screen of SCREENS) {
      ...
      screenshots.push({
        ...
        ...await captureRoute(browser, device, screen, `${APP_URL}${screen.path}`, output),
      });
    }
  ...
  await fs.writeFile(path.join(OUTPUT_DIR, "README.txt"), [
    "TheTok App Store — REAL restaurateur dashboard screenshots",
    "",
    `Source: ${APP_URL}`,
    ...
    "Mockups: not used.",
    `Dashboard pages: ${SCREENS.length}`,
    `Device formats: ${DEVICES.length}`,
    `Total screenshots: ${SCREENS.length * DEVICES.length}`,
    "",
...
async function restGet(table, params) {
  return jsonRequest(`${SUPABASE_URL}/rest/v1/${table}?${params.toString()}`, { headers: serviceHeaders() });
}
async function restInsert(table, payload, prefer = "return=representation") {
  return jsonRequest(`${SUPABASE_URL}/rest/v1/${table}`, {
    method: "POST",
...
async function restDelete(table, filter) {
  ...
  if (!response.ok && response.status !== 404) {
    ...
    throw new Error(`${table} cleanup returned HTTP ${response.status}: ${detail.slice(0, 300)}`);
  }
}

async function getElitePlan() {
  const select = [
    "id", "slug", "price_monthly_chf", "campaign_credit_chf", "ai_tool_credits", "ai_photo_credits",
    "monthly_conversation_limit", "monthly_text_tool_limit", "monthly_image_limit", "monthly_premium_image_limit",
    "monthly_voice_minutes_limit", "annual_months_charged", "acquired_reservation_fee_cents", "marketplace_commission_bps",
    "included_establishments", "additional_establishment_price_cents", "reservation_revenue_cap_bps", "developer_order_bps",
    "developer_tok_revenue_bps", "pricing_version",
  ].join(",");
  const params = new URLSearchParams({ select, slug: "eq.elite", is_active: "eq.true", limit: "1" });
  const rows = await restGet("restaurant_subscription_plans", params);
...
async function createTemporaryRestaurateurUser() {
  ...
  const created = await jsonRequest(`${SUPABASE_URL}/auth/v1/admin/users`, {
    ...
    body: JSON.stringify({
      ...
      password,
      email_confirm: true,
      user_metadata: { full_name: "Restaurant TOK", capture_only: true, legal_terms_accepted: true, privacy_policy_accepted: true },
      app_metadata: { capture_only: true },
    }),
  });
...
async function ensureUserRole(userId, role) {
  const params = new URLSearchParams({ select: "role", user_id: `eq.${userId}`, role: `eq.${role}`, limit: "1" });
  const existing = await restGet("user_roles", params);
...
async function upsertProfile(userId) {
  await restInsert("profiles", {
    user_id: userId,
    full_name: "Restaurant TOK",
    phone: "+41 22 000 00 00",
    city: "Espace interne",
    address: "Aperçu App Store",
...
async function createTemporaryRestaurant(userId) {
  const rows = await restInsert("restaurants", {
    owner_id: userId,
    name: "Restaurant TOK",
    description: "Espace restaurateur TOK",
    cuisine_type: "Cuisine suisse",
    address: "Aperçu App Store",
    city: "Espace interne",
    phone: "+41 22 000 00 00",
    price_range: 2,
    ...
    opening_hours: {
      monday: { open: "11:30", close: "22:00" }, tuesday: { open: "11:30", close: "22:00" },
      wednesday: { open: "11:30", close: "22:00" }, thursday: { open: "11:30", close: "22:00" },
      friday: { open: "11:30", close: "23:00" }, saturday: { open: "11:30", close: "23:00" }, sunday: { open: "11:30", close: "22:00" },
    },
  });
...
function pricingSnapshot(plan) {
  ...
  const billingVatCents = Math.round((priceMonthlyCents * vatRateBps) / (10_000 + vatRateBps));
  return {
    price_monthly_chf_snapshot: priceMonthlyCents / 100,
    billing_amount_chf_snapshot: priceMonthlyCents / 100,
    price_monthly_cents_snapshot: priceMonthlyCents,
    billing_amount_cents_snapshot: priceMonthlyCents,
    billing_net_cents_snapshot: priceMonthlyCents - billingVatCents,
    billing_vat_cents_snapshot: billingVatCents,
    vat_rate_bps_snapshot: vatRateBps,
    annual_months_charged_snapshot: Number(plan.annual_months_charged),
    acquired_reservation_fee_cents_snapshot: Number(plan.acquired_reservation_fee_cents),
    marketplace_commission_bps_snapshot: Number(plan.marketplace_commission_bps),
    included_establishments_snapshot: Number(plan.included_establishments),
    additional_establishment_price_cents_snapshot: plan.additional_establishment_price_cents == null ? null : Number(plan.additional_establishment_price_cents),
    reservation_revenue_cap_bps_snapshot: Number(plan.reservation_revenue_cap_bps),
    developer_order_bps_snapshot: Number(plan.developer_order_bps),
    developer_tok_revenue_bps_snapshot: Number(plan.developer_tok_revenue_bps),
    pricing_version_snapshot: String(plan.pricing_version),
  };
}
...
async function createTemporaryEliteSubscription(restaurantId, plan) {
  ...
  const rows = await restInsert("restaurant_ai_subscriptions", {
    ...
    metadata: { capture_only: true, source: "appstore-restaurateur-screenshot", payment_provider: "none" },
    ...pricingSnapshot(plan),
  });
  const subscription = Array.isArray(rows) ? rows[0] : null;
  if (!subscription?.id) throw new Error("Unable to create temporary Elite subscription");
  return subscription;
}
...
async function signInProduction(email, password) {
  ...
  if (!session?.access_token || !session?.refresh_token || !session?.user?.id) throw new Error("Production restaurateur sign-in did not return a complete session");
  return session;
}
...
async function installSessionAndPreferences(page, session, restaurantId) {
  ...
  const consent = {
    ...
    preferences: { necessary: true, analytics: false, marketing: false, personalization: false, geolocation: false },
    recordedAt: new Date().toISOString(),
    source: "settings",
  };
  await page.evaluate(({ authKey, session, consentKey, consent, restaurantId }) => {
    localStorage.setItem(authKey, JSON.stringify(session));
    localStorage.setItem(consentKey, JSON.stringify(consent));
    localStorage.setItem("miamz-dashboard-restaurant", restaurantId);
  }, { authKey, session, consentKey: CONSENT_STORAGE_KEY, consent, restaurantId });
...
async function gotoReady(page, url) {
  ...
  await page.waitForFunction(() => {
    ...
    return !text.includes("Chargement…") && !text.includes("Chargement...") && !text.includes("Ouverture de votre espace…");
  }, { timeout: 35_000 }).catch(() => undefined);
...
function assertRoute(finalUrl, expectedPath) {
  const finalPath = normalizePathname(finalUrl);
  const expected = expectedPath.replace(/\/+$/, "") || "/";
  if (finalPath !== expected) throw new Error(`Dashboard route redirected unexpectedly: expected ${expected}, got ${finalPath}`);
}
...
async function captureRoute(currentBrowser, device, screen, url, output) {
  ...
  try {
    ...
    assertRoute(page.url(), screen.path);
    const inspection = await page.evaluate(() => ({
      visibleText: document.body?.innerText || "",
      consentDialogOpen: Boolean(document.querySelector("#consent-title")),
      title: document.title || "",
    }));
    if (inspection.consentDialogOpen) throw new Error("Privacy consent dialog remained open during screenshot capture");
    if (/Accès refusé|Impossible d'ouvrir|Erreur inattendue|Page introuvable|Aucun restaurant associé/i.test(inspection.visibleText)) {
      throw new Error(`Screen failed to render: ${inspection.visibleText.replace(/\s+/g, " ").slice(0, 600)}`);
    }
    const buffer = await page.screenshot({ type: "jpeg", quality: 92, fullPage: false, captureBeyondViewport: false });
    await fs.writeFile(output, buffer);
    return {
      expected_pixel_width: device.width * device.dpr,
      expected_pixel_height: device.height * device.dpr,
      final_url: page.url(),
      document_title: inspection.title,
      visible_text_excerpt: inspection.visibleText.replace(/\s+/g, " ").trim().slice(0, 400),
...
async function cleanupRestaurant(restaurantId) {
  const restrictTables = [
    "restaurant_onboarding_state_events", "restaurant_stripe_adjustments", "reservation_fee_adjustments",
    "reservation_fee_charges", "reservation_flat_fee_charges", "payment_attempts", "financial_ledger",
    "commercial_earning_events", "promo_codes", "orders", "reservations", "order_groups", "order_items",
    "user_analytics", "user_subscriptions", "chef_table_drops",
  ];
  for (const table of restrictTables) await restDelete(table, `restaurant_id=eq.${restaurantId}`).catch((error) => console.warn(`Best-effort cleanup skipped for ${table}: ${safeError(error)}`));
  await restDelete("restaurant_ai_subscriptions", `restaurant_id=eq.${restaurantId}`);
  await restDelete("restaurants", `id=eq.${restaurantId}`);
}

async function cleanupUser(userId) {
  await restDelete("profiles", `user_id=eq.${userId}`).catch(() => undefined);
  await restDelete("user_roles", `user_id=eq.${userId}`).catch(() => undefined);
  const response = await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${userId}`, { method: "DELETE", headers: serviceHeaders() });
  if (!response.ok && response.status !== 404) throw new Error(`Auth cleanup returned HTTP ${response.status}`);
}
