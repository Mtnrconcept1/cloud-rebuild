    ...
    <AlertDialog open={!!conflict} onOpenChange={(open) => !open && resolveConflict("checkout")}>
      <AlertDialogContent className="rounded-2xl border-2">
        ...
        <AlertDialogFooter className="flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-start sm:space-x-0">
          {!isModeConflict && multiRestoEnabled && <AlertDialogAction onClick={() => { resolveConflict("clear"); navigate("/multi-restaurant"); }} className="h-auto w-full whitespace-normal rounded-xl bg-primary py-2.5 text-center font-bold text-primary-foreground hover:bg-[#b83200] sm:w-auto">Option Multi-Resto</AlertDialogAction>}
