    ...
    <AlertDialog open={!!conflict} onOpenChange={(open) => !open && resolveConflict("checkout")}>
      <AlertDialogContent className="rounded-2xl border-2">
        ...
        <AlertDialogFooter className="flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-start sm:space-x-0">
          {!isModeConflict && <AlertDialogAction onClick={() => { resolveConflict("clear"); navigate("/multi-restaurant"); }} className="w-full sm:w-auto rounded-xl bg-pink-500 hover:bg-pink-600 font-bold whitespace-normal text-center h-auto py-2.5">Option Multi-Resto</AlertDialogAction>}
