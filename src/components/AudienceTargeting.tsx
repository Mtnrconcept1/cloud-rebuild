...
export default function AudienceTargeting({ criteria, onChange, restaurantId }: AudienceTargetingProps) {
  const normalizedCriteria = useMemo(() => normalizeAudienceCriteria(criteria), [criteria]);
  ...
  const { data: cuisineOptions = [] } = useQuery({
    ...
    queryFn: async () => {
      const fallback = PREDEFINED_RESTAURANT_CATEGORIES.map((category) => category.name);
      const { data, error } = await (supabase.from("cuisines") as any).select("name").order("name");
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      setLoading(true);
      const estimateValue = await getAudienceEstimate({ ...normalizedCriteria, restaurantId });
  ...
  const update = (partial: Partial<AudienceCriteria>) => {
    onChange(normalizeAudienceCriteria({ ...normalizedCriteria, ...partial, restaurantId }));
  };

  const toggleListValue = (key: "cuisines" | "journeyTypes" | "serviceMoments", value: string) => {
      ...
      <Card className="bg-primary/5 border-primary/20">
        <CardContent className="py-4 flex items-center justify-between gap-4">
          ...
          <div className="text-right">
            <p className="text-2xl font-bold text-primary">{loading ? "..." : estimate.toLocaleString("fr-FR")}</p>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={normalizedCriteria.customerSegment} onValueChange={(value) => update({ customerSegment: value as AudienceCriteria["customerSegment"] })}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex items-center justify-between rounded-xl border p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium">Uniquement les fans du restaurant</p>
                <p className="text-xs text-muted-foreground">Cible les utilisateurs qui vous ont ajoute en favoris</p>
              </div>
            </div>
            <Switch checked={normalizedCriteria.favoritesOnly} onCheckedChange={(value) => update({ favoritesOnly: value })} />
          </div>
        </CardContent>
      </Card>
            ...
            {cuisineOptions.map((cuisine) => {
              ...
              return (
                <Toggle
                  ...
                  pressed={normalizedCriteria.cuisines.includes(normalizedCuisine)}
                  onPressedChange={() => toggleListValue("cuisines", normalizedCuisine)}
                  variant="outline"
                  size="sm"
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap gap-2">
            {JOURNEY_TYPE_OPTIONS.map((option) => (
              <Toggle
                ...
                onPressedChange={() => toggleListValue("journeyTypes", option.value)}
                variant="outline"
                size="sm"
                className="data-[state=on]:bg-primary data-[state=on]:text-primary-foreground"
              >
          ...
          <div className="flex flex-wrap gap-2">
            {SERVICE_MOMENT_OPTIONS.map((option) => (
              <Toggle
                key={option.value}
                pressed={normalizedCriteria.serviceMoments.includes(option.value)}
                onPressedChange={() => toggleListValue("serviceMoments", option.value)}
                variant="outline"
                size="sm"
                className="data-[state=on]:bg-primary data-[state=on]:text-primary-foreground"
              >
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline">{normalizedCriteria.minAvgBasket} CHF</Badge>
            </div>
            <Slider
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline">{normalizedCriteria.maxDaysSinceOrder} jours</Badge>
          </div>
          <Slider
