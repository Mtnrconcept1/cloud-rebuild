import { Calculator, Check, MapPinned, Menu } from "lucide-react";
      ...
      <div className="pointer-events-none fixed left-[calc(env(safe-area-inset-left,0px)+0.75rem)] top-[calc(env(safe-area-inset-top,0px)+0.75rem)] z-[1190]">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className="pointer-events-auto flex h-16 max-w-[min(18rem,calc(100vw-7rem))] items-center gap-2.5 rounded-[1.45rem] border border-orange-300/55 bg-zinc-950 px-2.5 pr-5 text-white shadow-[0_16px_34px_rgba(255,106,26,0.34),0_8px_24px_rgba(15,23,42,0.32)] ring-1 ring-white/15 backdrop-blur-md transition-transform hover:-translate-y-0.5"
              aria-label="Ouvrir le menu commercial"
            >
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[1rem] bg-gradient-to-br from-[#ff5a14] to-[#ffb000] text-white shadow-[0_0_24px_rgba(255,106,26,0.58)]">
          ...
          <DropdownMenuContent align="start" className="w-80 rounded-2xl p-2">
            ...
            {COMMERCIAL_NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <span className={cn("flex h-10 w-10 items-center justify-center rounded-xl bg-muted", selected && "bg-orange-500 text-white")}>
