...
export default function DropsManagement() {
  ...
  const createOrUpdateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: editingId ? "Drop mis a jour" : "Drop cree" });
      setEditingId(null);
      setForm(EMPTY_DROP);
    },
    onError: (error: any) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    createOrUpdateMutation.mutate({
      ...
      remaining_portions: editingId
        ? undefined
        : Number(form.total_portions || 0),
    ...
    <div className="container py-8 space-y-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Creez, modifiez et desactivez les drops Chef's Table.</p>
        </div>
      </div>

      <Card>
        <CardHeader><CardTitle>{editingId ? "Modifier le Drop" : "Nouveau Drop Chef's Table"}</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>Restaurant</Label>
              <select className="w-full rounded-md border p-2 text-sm" value={form.restaurant_id} onChange={(event) => setForm({ ...form, restaurant_id: event.target.value })}>
            ...
            <div className="space-y-2">
              <Label>Chef</Label>
              <Input value={form.chef_name} onChange={(event) => setForm({ ...form, chef_name: event.target.value })} placeholder="Nom du chef" required />
            </div>
            <div className="space-y-2">
              <Label>Plat</Label>
              <Input value={form.dish_name} onChange={(event) => setForm({ ...form, dish_name: event.target.value })} placeholder="Nom du plat" required />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input type="number" step="0.01" value={form.original_price} onChange={(event) => setForm({ ...form, original_price: event.target.value })} />
            </div>
            <div className="space-y-2">
              <Label>Portions</Label>
              <Input type="number" value={form.total_portions} onChange={(event) => setForm({ ...form, total_portions: event.target.value })} required />
            </div>
            <div className="space-y-2">
              <Label>Heure du Drop</Label>
              <Input type="datetime-local" value={form.drop_time} onChange={(event) => setForm({ ...form, drop_time: event.target.value })} required />
            </div>
            <div className="space-y-2">
            ...
            <label className="md:col-span-2 flex items-center gap-2 rounded-lg border px-3 py-2 text-sm">
              <input type="checkbox" checked={form.is_active} onChange={() => setForm({ ...form, is_active: !form.is_active })} />
              <span>Drop actif</span>
            </label>
            <div className="md:col-span-2 flex gap-2">
              <Button className="flex-1 bg-pink-500 hover:bg-pink-600" disabled={createOrUpdateMutation.isPending}>
                <Plus className="h-4 w-4 mr-2" />
                {editingId ? "Mettre a jour le Drop" : "Creer le Drop"}
              </Button>
              {editingId ? (
                <Button type="button" variant="outline" onClick={() => { setEditingId(null); setForm(EMPTY_DROP); }}>
                  Annuler
                </Button>
              ) : null}
            </div>
      ...
      <div className="grid grid-cols-1 gap-4">
          ...
          <Card key={drop.id}>
            <CardContent className="flex items-center justify-between gap-4 p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Restant: {drop.remaining_portions}/{drop.total_portions} | {new Date(drop.drop_time).toLocaleString()}
                  </p>
                  <p className="text-xs text-muted-foreground">{drop.is_active ? "Actif" : "Inactif"}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="icon" className="text-destructive" onClick={() => deleteDropMutation.mutate(drop.id)}>
        ...
        {drops.length === 0 ? <p className="text-sm text-muted-foreground">Aucun drop defini.</p> : null}
      </div>
    </div>
  );
}
