...
function buildSignupConfirmationRedirect(operationId?: string | null) {
  ...
  if (operationId)
    redirect.searchParams.set("signup_operation_id", operationId);
  ...
  if (role === "restaurateur") {
    ...
    if (
      !["monthly", "yearly"].includes(
        onboardingChoices.subscriptionBillingPeriod,
      )
    ) {
...
function getSignupApplicationForm(
  form: SignupFormState,
): SignupApplicationFormState {
...
async function finalizeSignupApplication(
  ...
  if (payload.role !== "client" || options.skipIfExisting) {
    ...
    if (existingApplication) {
      if (payload.role !== "client") {
        ...
        if (error && !String(error.message).includes("signup_draft_not_found"))
          throw error;
      }
      return;
    }
  ...
  const uploadedDocuments: UploadedSignupDocument[] =
    await uploadVerificationDocumentsWithRollback(
      getRequiredSignupDocuments(payload.role, payload.form.vehicleType).map(
        (requirement) => {
  ...
  const { error: syncError } = await supabase.rpc("sync_signup_application", {
    ...
    p_legal_name:
      payload.role === "restaurateur" ? payload.form.legalName : null,
    p_business_name:
      payload.role === "restaurateur" ? payload.form.businessName : null,
    p_business_registration_number:
      payload.role === "restaurateur"
        ? payload.form.businessRegistrationNumber
        : null,
    p_tax_id: payload.role === "restaurateur" ? payload.form.taxId : null,
    p_restaurant_name:
      payload.role === "restaurateur" ? payload.form.restaurantName : null,
    p_restaurant_description:
      payload.role === "restaurateur"
        ? payload.form.restaurantDescription
        : null,
    p_vehicle_type:
      payload.role === "courier" ? payload.form.vehicleType : null,
    p_license_plate:
      payload.role === "courier" ? payload.form.licensePlate : null,
    p_iban:
      payload.role === "courier" || payload.role === "restaurateur"
        ? payload.form.iban
        : null,
    p_metadata:
        ...
        : payload.role === "restaurateur"
          ? {
              ...
              contract_signer_name:
                payload.contractSignature?.signerName.trim() || null,
              contract_signature_data_url:
                payload.contractSignature?.signatureDataUrl || null,
  ...
  if (syncError) {
    const uploadedPaths = uploadedDocuments.map(
...
async function startRestaurantCardRegistrationAfterSignup(
  ...
  if (!application?.id || !restaurantId) {
    throw new Error(
      "Le dossier restaurateur est enregistré, mais l'enregistrement de la carte doit être repris depuis le dashboard.",
    );
  }
  ...
  if (!checkout.url)
    throw new Error(
      "La session d'enregistrement de la carte est en cours de vérification.",
    );
...
export default function Auth({ demoMode = false }: { demoMode?: boolean }) {
  ...
  const [commercialReferralToken, setCommercialReferralToken] = useState(() => {
    ...
    try {
      return String(
        window.sessionStorage.getItem(COMMERCIAL_REFERRAL_SESSION_KEY) || "",
      ).trim();
    } catch {
      return "";
    }
  });
  const requestedSubscriptionPlanSlug = String(
    searchParams.get("subscriptionPlan") || "",
  )
    .trim()
    .toLowerCase();
  const confirmationCompleted = searchParams.get("confirmed") === "1";
  const confirmationOperationId = String(
    searchParams.get("signup_operation_id") || "",
  ).trim();
  ...
  const [privilegedSignupSubmitting, setPrivilegedSignupSubmitting] =
    useState(false);
  const [privilegedSignupResumeChecking, setPrivilegedSignupResumeChecking] =
    useState(false);
  const [privilegedSignupAwaitingEmail, setPrivilegedSignupAwaitingEmail] =
    useState(false);
  const [selectedSubscriptionPlanId, setSelectedSubscriptionPlanId] =
    useState("");
  const [
    selectedSubscriptionBillingPeriod,
    setSelectedSubscriptionBillingPeriod,
  ...
  const privilegedSignupOperationRef = useRef<{
    key: string;
    id: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!confirmationNoticeShownRef.current) {
      ...
      toast({
        ...
        description:
          "Votre dossier restaurateur est conservé. La connexion finalisera automatiquement l’inscription.",
      });
    }
  ...
  const showExtendedIdentityFields =
    !isLogin && (roleMode === "restaurateur" || roleMode === "courier");
  ...
  const switchableRoles = useMemo(() => {
    ...
    if (isDemoAuthMode)
      return visibleRoles.filter(
        (candidateRole) => candidateRole === "restaurateur",
      );
    if (!isCommercialAuthHost) return visibleRoles;
    return visibleRoles.filter(
      (candidateRole) =>
        candidateRole === "commercial" || candidateRole === "admin",
    );
  ...
  const isRecoveringPrivilegedSignup = Boolean(
    user && session && incompletePrivilegedSignupRole,
  );
  ...
  useEffect(() => {
    ...
    try {
      window.sessionStorage.setItem(
        COMMERCIAL_REFERRAL_SESSION_KEY,
    ...
    const completeAuthRedirect = async () => {
      ...
      try {
        if (redirectState.error) {
          throw new Error(
            redirectState.errorDescription || redirectState.error,
          );
        }
        ...
        if (redirectState.code) {
          ...
          throw new Error(
            "Lien de connexion non compatible avec le flux sécurisé.",
          );
        }
      } catch {
        if (!cancelled) {
          toast({
            ...
            description:
              "Reconnectez-vous pour finaliser une session sécurisée.",
            variant: "destructive",
          });
        }
      } finally {
  ...
  useEffect(() => {
    if (!user?.id || !session || privilegedSignupResumeRef.current === user.id)
      return;
    ...
    const resumePrivilegedSignup = async () => {
      ...
      try {
        ...
        if (draft.status === "finalized") {
          await removePrivilegedSignupRecoveryDraft(draft.operation_id).catch(
            () => false,
          );
          await refreshRoles();
        ...
        const loadedRecovery = await loadPrivilegedSignupRecoveryDraft(
          draft.operation_id,
        ).catch(() => null);
        const recovery =
          loadedRecovery &&
          ...
          (!user.email ||
            loadedRecovery.email === user.email.trim().toLowerCase())
            ? loadedRecovery
            : null;
        ...
        const recoveredForm: SignupFormState = {
          ...
          fullName:
            localForm?.fullName ||
            signupForm.fullName ||
            String(safe.full_name || user.user_metadata?.full_name || ""),
          email: String(
            user.email || localForm?.email || signupForm.email || "",
          ),
          password: "",
          ...
          businessName:
            ...
            String(safe.business_name || ""),
          legalName: localForm?.legalName || signupForm.legalName,
          businessRegistrationNumber:
            localForm?.businessRegistrationNumber ||
            signupForm.businessRegistrationNumber,
          taxId: localForm?.taxId || signupForm.taxId,
          restaurantName:
            localForm?.restaurantName ||
            signupForm.restaurantName ||
            String(safe.restaurant_name || ""),
          restaurantDescription:
            localForm?.restaurantDescription ||
            signupForm.restaurantDescription ||
            String(safe.restaurant_description || ""),
          vehicleType:
            localForm?.vehicleType ||
            signupForm.vehicleType ||
            String(safe.vehicle_type || "bicycle"),
        ...
        const recoveredDocuments = recovery?.documents
          ...
          : documents;
        const recoveredOnboardingChoices:
        ...
        const recoveredLegalAcceptance: SignupLegalAcceptance = {
          ...
          acceptedAt,
          version:
            recovery?.legalAcceptance.version || LEGAL_ACCEPTANCE_VERSION,
        };
        const recoveredContractSignature =
          draft.requested_role === "restaurateur"
            ? {
                signerName:
                  recovery?.contractSignature?.signerName || contractSignerName,
                signatureDataUrl:
                  recovery?.contractSignature?.signatureDataUrl ||
        ...
        if (missingForm || missingDocuments.length > 0) {
          ...
          if (recoveredOnboardingChoices) {
            setSelectedSubscriptionPlanId(
              recoveredOnboardingChoices.subscriptionPlanId,
            );
            setSelectedSubscriptionBillingPeriod(
              recoveredOnboardingChoices.subscriptionBillingPeriod,
            );
          }
          if (recoveredContractSignature) {
            setContractSignerName(recoveredContractSignature.signerName);
            setContractSignatureDataUrl(
              recoveredContractSignature.signatureDataUrl,
            );
          }
          if (recoveredReferralToken)
            setCommercialReferralToken(recoveredReferralToken);
          setIsLogin(false);
        ...
        const contractContentSha256 =
          draft.requested_role === "restaurateur" && recoveredContractSignature
            ? await generateRestaurantPartnerContractSha256({
                ...
                businessRegistrationNumber:
                  recoveredForm.businessRegistrationNumber,
                ...
                selectedSubscriptionPlanLabel:
                  recovery?.selectedSubscriptionPlanLabel || null,
                selectedSubscriptionPriceLabel:
                  recovery?.selectedSubscriptionPriceLabel || null,
              })
            : null;
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Impossible de reprendre l'inscription.";
        toast({
          title: "Inscription à reprendre",
          description: message,
          variant: "destructive",
  ...
  useEffect(() => {
    ...
    if (!incompleteSignupNoticeRef.current) {
      ...
      toast({
        ...
        description:
          "Votre compte est confirmé. Ajoutez les informations et documents manquants pour créer votre espace en attente.",
      });
    }
  }, [
    incompletePrivilegedSignupRole,
  ...
  useEffect(() => {
    ...
    const referredPlan =
      commercialReferralToken && requestedSubscriptionPlanSlug
        ? subscriptionPlans.find(
            (plan) => plan.slug === requestedSubscriptionPlanSlug,
          )
        : null;
    ...
    const { error } = await supabase.auth.resend({
      ...
      options: {
        emailRedirectTo: buildSignupConfirmationRedirect(
          privilegedSignupOperationRef.current?.id ||
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const isContinuingConfirmedSignup = Boolean(
        user?.id && session && incompletePrivilegedSignupRole === submittedRole,
      );
      ...
      const operationKey =
        isContinuingConfirmedSignup && user?.id
          ...
          recoveryDraftSaved = await savePrivilegedSignupRecoveryDraft({
            ...
            contractSignature:
              submittedRole === "restaurateur"
                ...
                : undefined,
            selectedSubscriptionPlanLabel:
              selectedSubscriptionPlan?.name || null,
            selectedSubscriptionPriceLabel:
      ...
      if (!isContinuingConfirmedSignup) {
                ...
                ? {
                    ...
                    signup_business_name:
                      submittedRole === "restaurateur"
                        ? signupForm.businessName
                        : null,
                    signup_restaurant_name:
                      submittedRole === "restaurateur"
                        ? signupForm.restaurantName
                        : null,
                    signup_restaurant_description:
                      submittedRole === "restaurateur"
                        ? signupForm.restaurantDescription
                        : null,
                    signup_vehicle_type:
        ...
        if (signUpResponse.error) {
          if (isPrivilegedSignup) {
            await removePrivilegedSignupRecoveryDraft(operationId).catch(
              () => false,
            );
          }
      ...
      if (isPrivilegedSignup) {
        privilegedSignupOperationRef.current = {
          key: `${activeUser.id}:${submittedRole}`,
          id: operationId,
        };
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-secondary/10 px-4 py-10">
      <Card className="w-full max-w-3xl shadow-lg border-0">
        ...
        <CardContent className="space-y-6">
          {confirmationCompleted && !user ? (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
              ...
              <p className="pt-1 text-emerald-800">
                Connectez-vous si la session ne s’est pas ouverte
                automatiquement. Le dossier restaurateur sera restauré sans
                ressaisie.
              </p>
            </div>
          ) : null}
          {isLogin &&
          (postAuthRedirectTarget ||
            searchParams.get("domain") === "required") ? (
            ...
            <div className="flex items-start gap-3 rounded-xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-950 dark:border-violet-900/60 dark:bg-violet-950/30 dark:text-violet-100">
              ...
              <div>
                ...
                <p className="mt-1 text-xs opacity-80">
                  Aucune commande, réservation, facture ou donnée de ce compte
                  ne peut atteindre la production.
                </p>
              </div>
            </div>
          ) : null}
          ...
          {forgotPassword ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4 rounded-2xl border bg-card/60 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Ce choix est joint au dossier et une facture
                        d’abonnement est réservée dès l’inscription. La carte
                        sera enregistrée depuis le dashboard, sans débit
                        immédiat.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        type="button"
                        variant={
                          selectedSubscriptionBillingPeriod === "monthly"
                            ? "default"
                            : "outline"
                        }
                        onClick={() =>
                          setSelectedSubscriptionBillingPeriod("monthly")
                        }
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setSelectedSubscriptionBillingPeriod("yearly")
                        }
                        disabled={
                          !annualBillingEnabled ||
                          Boolean(commercialReferralToken)
                        }
                        title={
                          annualBillingEnabled
                            ? "12 mois de service au prix de 11"
                            : "Activation après validation Stripe"
                        }
                      >
                        Annuel · 11 pour 12
                      </Button>
                    ...
                    {!annualBillingEnabled ? (
                      <p className="text-xs text-muted-foreground">
                        L'annuel sera activé après validation du parcours Stripe
                        complet. Les montants sont déjà affichés à titre
                        contractuel.
                      </p>
                    ) : null}
                    ...
                    {subscriptionPlansLoading ? (
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        {subscriptionPlans.map((plan) => {
                          ...
                          const amount =
                            fairGrowthPlan.monthlyPriceChf *
                            (selectedSubscriptionBillingPeriod === "yearly"
                              ? FAIR_GROWTH_ANNUAL_MONTHS_CHARGED
                              : 1);
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="mt-2 grid gap-1 rounded-lg bg-primary/5 p-2 text-xs text-muted-foreground">
                                <span>
                                  <strong>
                                    {formatChf(RESERVATION_FLAT_FEE_CHF)}
                                  </strong>{" "}
                                  / réservation honorée
                                </span>
                                <span>
                                  <strong>
                                    {(
                                      fairGrowthPlan.marketplaceCommissionBps /
                                      100
                                ...
                                <span>
                                  Canaux propres, annulation, no-show et
                                  remboursement : <strong>CHF 0</strong>
                                </span>
                                <span>
                                  Plafond réservation :{" "}
                                  <strong>7% du CA table</strong>
                                </span>
                                <span>
                                  Au minimum 90% au restaurant + 100% des
                                  pourboires
                                </span>
                                {fairGrowthPlan.slug === "elite" ? (
                                  <>
                                    <span>
                                      3 établissements inclus · CHF 149/site
                                      supplémentaire
                                    </span>
                                    <span>
                                      Sites rattachés après validation TOK ;
                                      aucun supplément sans confirmation.
                                    </span>
                                  </>
                                ) : null}
                              </span>
                              <span className="mt-3 grid gap-1 text-xs text-muted-foreground">
                                <span className="font-semibold text-foreground">
                                  {formatTokCredits(tokCredits)} / mois
                                </span>
                                <span>
                                  Utilisables pour campagnes, IA, photos et
                                  visuels.
                                </span>
                                <span>
                                  ou{" "}
                  ...
                  {selectedSubscriptionPlan ? (
                    <div className="rounded-xl border bg-background p-3 text-sm">
                      <p className="font-medium">
                        Snapshot Fair Growth réservé au dossier
                      </p>
                      <p className="pt-1 text-muted-foreground">
                        {formatChf(selectedSubscriptionPrice)}{" "}
                        {selectedSubscriptionBillingPeriod === "yearly"
                          ? "pour 12 mois de service (11 mois facturés)"
                          : "/ mois"}
                        . Le plan, la période, les taux et la version tarifaire
                        seront scellés côté serveur.
                      </p>
                    </div>
                  ) : null}
                </div>
                ...
                >
                  <div className="pointer-events-auto max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto overscroll-contain rounded-3xl border border-slate-200 bg-white p-5 text-slate-950 shadow-2xl sm:p-7">
                    ...
                    <div className="mt-6 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left text-sm">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setLegalAcceptanceDraft(checked === true)
                        }
                    ...
                    <p className="mt-4 text-center text-xs leading-5 text-slate-500">
                      Cochez la case, puis confirmez votre choix. La fenêtre
                      reste ouverte tant que vous n'avez pas accepté ou refusé.
                    </p>
              ...
              {privilegedSignupAwaitingEmail && !session ? (
                <div
                  className="space-y-3 rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-950"
                  role="status"
                >
                  <p>
                    Confirmez votre email. Votre formulaire et vos documents
                    restent conservés localement sur cet appareil.
                  </p>
