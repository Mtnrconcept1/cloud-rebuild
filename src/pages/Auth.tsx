...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-secondary/10 px-4">
      <Card className="w-full max-w-md shadow-lg border-0">
        <CardHeader className="text-center space-y-2">
          <img src={LOGO_URL} alt="Tok" className="mx-auto h-20 w-auto object-contain mb-2" />
          <CardTitle className="font-display text-2xl">{isLogin ? "Bon retour !" : "Créer un compte"}</CardTitle>
          <CardDescription>{isLogin ? "Connectez-vous pour accéder à vos restaurants favoris" : "Rejoignez Tok et découvrez les meilleurs restaurants"}</CardDescription>
        </CardHeader>
        <CardContent>
          {!isLogin && (
