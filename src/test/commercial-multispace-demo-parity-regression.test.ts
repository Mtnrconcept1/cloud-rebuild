...
describe("commercial multi-space demo parity", () => {
  ...
  it("removes presentation quotas only in the dedicated demo database while preserving safety gates", () => {
    ...
    expect(migration).toContain("provider_failure_circuit_open");
    expect(migration).toContain("v_processing_count >= 12");
    expect(migration).toContain("v_session_processing_count >= 2");
