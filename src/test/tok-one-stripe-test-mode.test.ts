...
describe("Tok One Stripe test mode", () => {
  ...
  it("keeps test checkout-session sync pinned to a test Stripe runtime with compatible fallbacks", () => {
    ...
    expect(testRuntimeBlock).toContain('"STRIPE_PERSONNAL_SECRET_KEY"');
    expect(testRuntimeBlock).toContain('"STRIPE_PERSONAL_SECRET_KEY"');
    expect(testRuntimeBlock).toContain('"STRIPE_SECRET_KEY"');
    expect(testRuntimeBlock).toContain('"STRIPE_SECRET_KEY_LIVE"');
  ...
  it("syncs the new Tok One Stripe secret names in production without storing values", () => {
    ...
    expect(workflowSource).toContain("STRIPE_SECRET_KEY: ${{ secrets.STRIPE_PERSONNAL_SECRET_KEY || secrets.STRIPE_PERSONAL_SECRET_KEY || secrets.STRIPE_SECRET_KEY_LIVE }}");
    expect(workflowSource).toContain("STRIPE_SECRET_KEY_LIVE: ${{ secrets.STRIPE_PERSONNAL_SECRET_KEY || secrets.STRIPE_PERSONAL_SECRET_KEY || secrets.STRIPE_SECRET_KEY_LIVE }}");
