...
describe("marketing server-only BFF", () => {
  it("uses the exact closed set of 24 marketing operations", () => {
    ...
    expect(
      MARKETING_OPERATION_NAMES.every((name) => name.startsWith("admin_")),
    ).toBe(true);
    expect(MARKETING_OPERATION_NAMES).toEqual(
  ...
  it("rejects non-marketing hosts and cross-site mutation metadata", () => {
    expect(() =>
      validateMarketingRequestContext(request(), false),
    ).not.toThrow();
    expect(() =>
      validateMarketingRequestContext(
        request({ host: "www.thetok.ch" }),
        false,
      ),
    ).toThrow();
    expect(() =>
  ...
  it("matches the hardened SQL RPC contract and never sets token cookies", () => {
    const source = readFileSync(
      resolve(process.cwd(), "server/marketingBff.ts"),
      "utf8",
    );
    ...
    expect(source).not.toMatch(
      /p_pending_token|p_session_token|p_csrf_token|p_require_csrf/,
    );
    expect(source).not.toMatch(
  ...
  it("keeps password-login Supabase tokens server-side", async () => {
    ...
    expect(recorder.body).not.toMatch(
      /supabase-access-token|supabase-refresh-token/,
    );
    const cookies = recorder.headers.get("set-cookie") as readonly string[];
    expect(
      cookies.some(
        (value) =>
          value.startsWith("__Host-tok_marketing_pending=") &&
          value.includes("HttpOnly"),
      ),
    ).toBe(true);
    expect(cookies.join(";")).not.toMatch(
      /supabase-access-token|supabase-refresh-token/,
    );
  });
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        if (url.endsWith("/auth/v1/factors")) {
          return json({
            ...
            totp: {
              qr_code:
                'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg"><text>100%</text></svg>',
            },
          });
        }
        if (url.endsWith(`/auth/v1/factors/${factorId}/challenge`)) {
  ...
  it("refuses AAL1 TOTP bootstrap when another verified factor exists", async () => {
    ...
    expect(requestedUrls.some((url) => url.endsWith("/auth/v1/factors"))).toBe(
      false,
    );
    expect(
      requestedUrls.some((url) =>
        url.endsWith("/rpc/service_store_marketing_auth_challenge"),
      ),
    ).toBe(false);
    expect(
      requestedUrls.some((url) => url.endsWith("/auth/v1/logout?scope=local")),
    ).toBe(true);
  });
  ...
  it("finalizes only a fresh AAL2 result into opaque cookies", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
        ...
        if (url.endsWith("/rpc/service_revoke_marketing_web_session"))
          return json(true);
        throw new Error(`Unexpected test request: ${url}`);
      }),
    );
    ...
    const cookies = recorder.headers.get("set-cookie") as readonly string[];
    const sid = cookies.find((value) =>
      value.startsWith("__Host-tok_marketing_sid="),
    );
    ...
    expect(csrf).not.toContain("HttpOnly");
    expect(cookies.join(";")).not.toMatch(
      /aal2-refresh-token|aal1-access-token/,
    );
    expect(consumedRateKeys.size).toBe(2);
    for (const key of consumedRateKeys)
      expect(clearedRateKeys.has(key)).toBe(true);
  });
