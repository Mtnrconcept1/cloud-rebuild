...
import {
  ...
  RESTAURANT_ONBOARDING_ROUTE_CATALOG,
...
describe("restaurant onboarding subscription payments", () => {
  it.each([
    [
      "accepted card",
      { paymentAttemptState: "finalized", setupIntentStatus: "succeeded" },
      "webhook_received",
    ],
    [
      "3-D Secure",
    ...
    [
      ...
      { paymentAttemptState: "finalized", webhookReceived: true },
      "webhook_received",
    ],
  ] as const)("resolves the server state for %s", (_name, input, expected) => {
    expect(resolveRestaurantOnboardingAttemptView(input)).toMatchObject({
      state: expected,
    });
  });
  ...
  it("keeps replayed webhooks and browser retries terminal and idempotent", () => {
    const first = resolveRestaurantOnboardingAttemptView({
      paymentAttemptState: "finalized",
      webhookReceived: true,
    });
    const replay = resolveRestaurantOnboardingAttemptView({
      paymentAttemptState: "finalized",
      webhookReceived: true,
      browserReturnReceived: true,
    });
  ...
  it("allows every catalogued preparation tab and rejects transaction routes", () => {
    for (const { path } of RESTAURANT_ONBOARDING_ROUTE_CATALOG) {
      expect(
        canAccessRestaurantDashboardRoute({
          pathname: path,
          dashboardAccessLocked: true,
          onboardingConfigurationUnlocked: true,
  ...
  it("enforces the feature flag for a catalogued onboarding route", () => {
    expect(
      canAccessRestaurantDashboardRoute({
        ...
        dashboardAccessLocked: true,
        onboardingConfigurationUnlocked: true,
        disabledFeatures: new Set(["dashboard-menu"]),
      }),
    ).toBe(false);
  });
  ...
  it("collects subscription choices during restaurateur signup", () => {
    ...
    expect(auth).toContain(
      'buildCheckoutReturnUrl("/dashboard", { paymentAttemptId })',
    );
  ...
  it("unlocks only restaurant configuration after the card is ready and before admin approval", () => {
    ...
    expect(layout).toContain(
      "isRestaurantOnboardingConfigurationRoute(pathname)",
    );
    expect(layout).toContain(
      "Les outils liés à l'abonnement seront disponibles après l'approbation de l'admin TOK.",
    );
  ...
  it("saves a card during onboarding and defers the Stripe subscription", () => {
    ...
    expect(checkout).toContain("signup_application_id requis");
    ...
    expect(webhook).toContain(
      "restaurant_subscription_stale_invoice_failure_ignored",
    );
  ...
  it("blocks admin approval until the reusable payment method is ready", () => {
    ...
    expect(paymentArchitectureMigration).toContain(
      "get_commercial_prospect_signup_referral",
    );
    expect(paymentArchitectureMigration).toContain(
      "commercial_signup_referral",
    );
    expect(paymentArchitectureMigration).toContain(
      "deferred_subscription_contract_snapshot_is_immutable",
    );
    ...
    expect(paymentArchitectureMigration).toContain(
      "queue_subscription_activation_on_reservation",
    );
    expect(paymentArchitectureMigration).toContain(
      "queue_subscription_activation_on_order",
    );
    expect(paymentArchitectureMigration).toContain(
      "record_restaurant_subscription_invoice_paid",
    );
    expect(paymentArchitectureMigration).not.toContain("v_launch_pack_id");
    expect(paymentArchitectureMigration).not.toContain(
      "restaurant_launch_packs rlp",
    );
    ...
    expect(statusCard).toContain(
      "TOK ne débite aucun abonnement à l’inscription",
    );
    expect(statusCard).toContain(
      "Votre banque peut éventuellement afficher une vérification temporaire de la carte",
    );
