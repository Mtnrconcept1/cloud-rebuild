-- Fix signup lifecycle:
-- - client signup has no verification document requirement and is immediately usable
-- - restaurateur/courier roles are granted only after admin approval
-- - admin review can approve, request changes, or reject directly from the app
...
WHERE ur.user_id = sa.user_id
  AND ur.role = sa.requested_role
  AND sa.requested_role IN ('restaurateur', 'courier')
  AND sa.status IN ('pending_review', 'needs_changes', 'rejected')
  AND NOT EXISTS (
    SELECT 1
    FROM public.signup_applications approved_sa
    WHERE approved_sa.user_id = sa.user_id
      AND approved_sa.requested_role = sa.requested_role
      AND approved_sa.status = 'approved'
  );
