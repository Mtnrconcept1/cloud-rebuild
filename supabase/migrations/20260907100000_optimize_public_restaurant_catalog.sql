...
-- The public city/search RPC used to score thousands of directory rows before
-- applying visibility and image constraints. Filter the real public catalogue
-- first, then run the expensive enrichment only on that bounded candidate set.
CREATE INDEX IF NOT EXISTS idx_restaurants_public_catalog_normalized_city_trgm
...
BEGIN
  ...
  WITH candidate_restaurants AS MATERIALIZED (
    ...
    WHERE r.is_active IS TRUE
      ...
      AND NULLIF(btrim(r.image_url), '') IS NOT NULL
...
-- Preserve the row-returning contract used by local city pages while making it
-- consume the same candidate-first pagination path.
...
RETURNS TABLE(
  ...
  review_count integer, price_range integer, delivery_fee numeric, image_url text,
  address text, city text, delivery_available boolean, created_at timestamp with time zone,
  category_names text[], category_slugs text[], matched_via_menu boolean,
  monthly_reservations integer, monthly_orders integer, promotion_score numeric,
  relevance_score numeric, latitude double precision, longitude double precision
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
  WITH payload AS (
    SELECT page.items
    FROM public.search_restaurants_catalog_page(
      left(p_query, 160), left(p_city, 120), left(p_cuisine, 120),
      p_price_range, p_delivery_only, p_min_rating,
      left(p_sort_by, 32), left(p_sort_direction, 8),
      LEAST(GREATEST(COALESCE(p_limit, 54), 1), 54),
      LEAST(GREATEST(COALESCE(p_offset, 0), 0), 1000000)
    ) AS page
  )
  SELECT
    (entry.item ->> 'id')::uuid,
    entry.item ->> 'name', entry.item ->> 'description', entry.item ->> 'cuisine_type',
    COALESCE((entry.item ->> 'rating')::numeric, 0),
    COALESCE((entry.item ->> 'review_count')::integer, 0),
    COALESCE((entry.item ->> 'price_range')::integer, 2),
    COALESCE((entry.item ->> 'delivery_fee')::numeric, 0),
    entry.item ->> 'image_url', entry.item ->> 'address', entry.item ->> 'city',
    COALESCE((entry.item ->> 'delivery_available')::boolean, false),
    (entry.item ->> 'created_at')::timestamp with time zone,
    ARRAY(SELECT jsonb_array_elements_text(COALESCE(entry.item -> 'category_names', '[]'::jsonb))),
    ARRAY(SELECT jsonb_array_elements_text(COALESCE(entry.item -> 'category_slugs', '[]'::jsonb))),
    COALESCE((entry.item ->> 'matched_via_menu')::boolean, false),
    COALESCE((entry.item ->> 'monthly_reservations')::integer, 0),
    COALESCE((entry.item ->> 'monthly_orders')::integer, 0),
    COALESCE((entry.item ->> 'promotion_score')::numeric, 0),
    COALESCE((entry.item ->> 'relevance_score')::numeric, 0),
    (entry.item ->> 'latitude')::double precision,
    (entry.item ->> 'longitude')::double precision
  FROM payload
  CROSS JOIN LATERAL jsonb_array_elements(payload.items) WITH ORDINALITY AS entry(item, position)
...
COMMENT ON FUNCTION public.search_restaurants_catalog_page(
  text, text, text, integer, boolean, numeric, text, text, integer, integer
) IS 'Exact image-backed public catalogue pagination with candidate-first filtering, capped at 54 rows.';
COMMENT ON FUNCTION public.search_restaurants_catalog(
  text, text, text, integer, boolean, numeric, text, text, integer, integer
) IS 'Compatibility row wrapper over the candidate-first public catalogue pagination.';
