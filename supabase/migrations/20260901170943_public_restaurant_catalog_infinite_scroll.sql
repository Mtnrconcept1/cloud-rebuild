-- Return complete, deterministic public restaurant pages for infinite scrolling.
-- The historical public wrapper applied visibility after LIMIT/OFFSET, which
-- could return fewer rows than requested and incorrectly signal the end of the
-- catalogue. This additive RPC filters all candidates first, then counts and
-- paginates the visible result set in stable groups of at most 54.
BEGIN;
    ...
    SELECT
      ...
      row_number() OVER (
        ORDER BY
          ...
          END ASC,
          CASE
            WHEN controls.sort_by = 'pertinence' AND controls.sort_direction = 'desc'
              THEN visible.relevance_score
          END DESC,
          CASE
            WHEN controls.sort_by = 'pertinence' AND controls.sort_direction = 'asc'
              THEN visible.relevance_score
          END ASC,
          visible.rating DESC,
...
COMMENT ON FUNCTION public.search_restaurants_catalog_page(
  ...
  'Returns an exact visible restaurant count and a deterministic public page capped at 54 rows.';
