...
-- reservation. Historical pending/confirmed cases stay untouched; only
-- terminally invalid credits are compensated below.
...
declare
  ...
  v_reward public.loyalty_transactions%rowtype;
...
begin
  ...
  if v_disqualifying and found then
    ...
    insert into public.loyalty_transactions (
      ...
      new.id,
      -abs(v_reward.amount),
      case when v_reward.transaction_type = 'donation'
      ...
      jsonb_build_object(
        'reverses_transaction_id', v_reward.id,
    ...
    if v_transaction_id is not null then
      if v_reward.transaction_type = 'donation' then
        insert into public.solidarity_donations (user_id, points_amount, meals_count)
        values (
          v_reward.user_id,
          -abs(v_reward.amount),
          -floor(abs(v_reward.amount)::numeric / 1000)::integer
        );
      else
        update public.profiles
        set loyalty_points = coalesce(loyalty_points, 0) - abs(v_reward.amount)
  ...
  if not v_eligible or found then return new; end if;
  ...
  v_restaurant_multiplier := coalesce(v_restaurant_multiplier, 1);
  ...
  if v_multiplier_text ~ '^[0-9]+([.][0-9]+)?$' then
    v_miamz_multiplier := greatest(1, v_multiplier_text::numeric);
  end if;
  v_effective_multiplier := v_restaurant_multiplier * v_miamz_multiplier;
...
begin
  ...
  if v_disqualifying and found then
    ...
    insert into public.loyalty_transactions (
      ...
      new.id,
      -abs(v_reward.amount),
      case when lower(coalesce(v_reward.metadata ->> 'donated', 'false')) in ('1', 'true', 'yes', 'oui')
    ...
    returning id into v_transaction_id;
    if v_transaction_id is not null then
      if lower(coalesce(v_reward.metadata ->> 'donated', 'false')) in ('1', 'true', 'yes', 'oui') then
        insert into public.solidarity_donations (user_id, points_amount, meals_count)
        values (
          v_reward.user_id,
          -abs(v_reward.amount),
...
-- Abort atomically if compensation of certain terminal failures would make an
-- existing profile negative. Missing legacy profiles are deliberately left for
-- manual review instead of manufacturing a balance.
do $$
begin
  if exists (
    with invalid_rewards as (
      ...
      where lt.transaction_type = 'order_earned'
        and (
          lower(coalesce(o.status, '')) in ('cancelled', 'refunded', 'payment_failed', 'refused')
          or lower(coalesce(o.payment_status, '')) in ('failed', 'refunded')
          or lower(coalesce(o.refund_status, '')) in ('refunded', 'fully_refunded', 'full_refund')
        )
        and not exists (
      ...
      where lt.transaction_type = 'reservation_earned'
        and (
          lower(coalesce(r.status, '')) in ('cancelled', 'no_show', 'refunded')
          or lower(coalesce(r.refund_status, '')) in ('refunded', 'fully_refunded', 'full_refund')
        )
        and not exists (
          select 1 from public.loyalty_transactions reversal
          where reversal.metadata ->> 'reverses_transaction_id' = lt.id::text
...
with candidates as (
  ...
  select user_id, order_id, -abs(amount), 'order_reward_reversal',
         ...
         jsonb_build_object(
           ...
           'reason', 'migration_terminal_invalid_reward',
           'status', status,
