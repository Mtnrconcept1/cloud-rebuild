-- Restore La Table du Chef checkout idempotency after the generic
-- reservation reference migration overwrote the dedicated paid-flow behavior.
-- Two guarantees are required:
-- 1. paid chefs_table reservations created by the service role are inserted
--    directly as confirmed
-- 2. concurrent finalization attempts (Stripe webhook + browser return) reuse
--    the same reservation instead of raising a duplicate reservation error
...
DECLARE
  ...
  v_existing_dup_paid boolean := false;
  v_existing_dup_total numeric := 0;
  v_existing_dup_meta jsonb := '{}'::jsonb;
BEGIN
