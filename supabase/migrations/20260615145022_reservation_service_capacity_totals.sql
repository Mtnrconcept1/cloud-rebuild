-- reservation-service-capacity: table capacity is shared across a full service.
-- If a restaurant configures 10 tables for dinner, those 10 tables are the
-- total dinner quota, not 10 tables repeated for each reservation time.
...
BEGIN
  ...
  v_service_table_capacity := COALESCE(public.get_reservation_slot_capacity(v_service_settings, p_time), 1);
  SELECT count(*)::integer INTO v_service_reserved_tables
  FROM public.reservations r
  WHERE r.restaurant_id = p_restaurant_id
    AND r.date = p_date
