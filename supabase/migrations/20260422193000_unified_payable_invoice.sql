-- Unified payable invoices (TOK -> restaurant) with normalized billed line items.
-- Goal:
-- 1. Keep payout invoices (TOK -> restaurant 90%) separate.
-- 2. Replace single-purpose reservation_fees invoices with a canonical payable
--    invoice model that can bill any payable item: commissions, reservation fees,
--    campaign payments, and future fees.

ALTER TABLE public.restaurant_invoices
  ALTER COLUMN invoice_type SET DEFAULT 'payout';
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.restaurant_invoices
      DROP CONSTRAINT restaurant_invoices_type_check;
  END IF;

  ALTER TABLE public.restaurant_invoices
    ADD CONSTRAINT restaurant_invoices_type_check
    CHECK (invoice_type IN ('payout', 'reservation_fees', 'payable'));
END $$;

CREATE TABLE IF NOT EXISTS public.restaurant_invoice_line_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  invoice_id uuid NOT NULL REFERENCES public.restaurant_invoices(id) ON DELETE CASCADE,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'restaurant_invoice_line_items_kind_check'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_restaurant_invoice_line_items_invoice_id
  ON public.restaurant_invoice_line_items(invoice_id, occurred_at, created_at);

CREATE INDEX IF NOT EXISTS idx_restaurant_invoice_line_items_restaurant_time
  ON public.restaurant_invoice_line_items(restaurant_id, occurred_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS idx_restaurant_invoice_line_items_unique_source_kind
  ON public.restaurant_invoice_line_items(source_table, source_id, item_kind)
  WHERE source_id IS NOT NULL;

ALTER TABLE public.restaurant_invoice_line_items ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "restaurant_invoice_line_items_owner_select" ON public.restaurant_invoice_line_items;
CREATE POLICY "restaurant_invoice_line_items_owner_select"
  ...
  USING (
    EXISTS (
      ...
      WHERE r.id = restaurant_invoice_line_items.restaurant_id
        AND r.owner_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "restaurant_invoice_line_items_admin_all" ON public.restaurant_invoice_line_items;
CREATE POLICY "restaurant_invoice_line_items_admin_all"
  ON public.restaurant_invoice_line_items
  FOR ALL
  TO authenticated
...
DROP TRIGGER IF EXISTS set_restaurant_invoice_line_items_updated_at ON public.restaurant_invoice_line_items;
CREATE TRIGGER set_restaurant_invoice_line_items_updated_at
  BEFORE UPDATE ON public.restaurant_invoice_line_items
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

CREATE OR REPLACE FUNCTION public.get_payable_invoice_lines(
  p_invoice_id uuid
)
RETURNS TABLE (
  line_id uuid,
...
BEGIN
  ...
  IF COALESCE(v_invoice.invoice_type, 'payout') <> 'payable' THEN
  ...
  ORDER BY li.occurred_at ASC, li.created_at ASC, li.id ASC;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.get_payable_invoice_lines(uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_payable_invoice_lines(uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_payable_invoice_lines(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_payable_invoice_lines(uuid) TO service_role;
...
BEGIN
  ...
  WHERE r.restaurant_id = p_restaurant_id
    AND r.date BETWEEN v_period_s AND v_period_e
...
BEGIN
  ...
  FOR v_restaurant IN
    ...
    FROM (
      ...
      FROM public.reservations r
      WHERE r.date BETWEEN v_period_s AND v_period_e
        AND COALESCE(r.total_amount, 0) > 0
        AND lower(COALESCE(r.status, '')) NOT IN ('cancelled', 'no_show', 'pending')
        AND NOT EXISTS (
          SELECT 1
