-- Fix privileged signup dossiers and exclusive roles.
-- Restaurant/courier registrations are public, email-gated dossiers reviewed by admins;
-- they must not receive an extra client role in admin user management.
...
-- Repair already-created privileged signups that were shown as both client and restaurateur/courier.
DELETE FROM public.user_roles ur
USING public.signup_applications sa
WHERE ur.user_id = sa.user_id
  AND ur.role = 'client'::public.app_role
  AND sa.requested_role IN ('restaurateur'::public.app_role, 'courier'::public.app_role);

-- Make dossiers visible even if the email was confirmed before the draft was written or the trigger was missed.
UPDATE public.signup_applications sa
SET status = 'pending_review', submitted_at = now(), updated_at = now()
FROM auth.users u
WHERE u.id = sa.user_id
  AND u.email_confirmed_at IS NOT NULL
  AND sa.status = 'awaiting_email';
