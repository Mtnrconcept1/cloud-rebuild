-- Actualites paid post distribution and reliable conversion attribution.
-- The feed uses a weighted lottery: a larger remaining paid budget increases the
-- probability of appearing near the top, while a 15-minute viewer-specific seed
-- prevents the same sponsored post from always winning.
...
BEGIN
  IF NEW.user_id IS NOT NULL
     ...
     AND COALESCE(NEW.payment_status, '') NOT IN ('payment_failed', 'failed', 'cancelled', 'refused', 'pending_payment') THEN
    PERFORM public.record_actualites_sponsored_conversion(
      NEW.user_id,
      NEW.restaurant_id,
      'order',
      NEW.id,
      COALESCE(NEW.metadata->>'payment_method', 'unknown')
    );
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS record_actualites_order_conversion_on_orders ON public.orders;
...
DECLARE
  v_feature text;
BEGIN
  v_feature := COALESCE(NEW.feature, NEW.metadata->>'feature', 'reservation');
  IF NEW.user_id IS NOT NULL
     AND NEW.restaurant_id IS NOT NULL
    ...
    PERFORM public.record_actualites_sponsored_conversion(
      NEW.user_id,
      NEW.restaurant_id,
      CASE WHEN v_feature IN ('zero-attente', 'zero_attente') THEN 'zero-attente' ELSE 'reservation' END,
      NEW.id,
      COALESCE(NEW.metadata->>'payment_method', 'unknown')
    );
  END IF;
...
DROP TRIGGER IF EXISTS record_actualites_reservation_conversion_on_reservations ON public.reservations;
