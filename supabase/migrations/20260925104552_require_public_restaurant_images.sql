...
BEGIN
  IF to_regclass('public.restaurants') IS NULL THEN
    RAISE EXCEPTION 'Required table public.restaurants is missing';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='restaurants' AND column_name='image_url'
  ) OR NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='restaurants' AND column_name='directory_image_verified'
  ) THEN
    RAISE EXCEPTION 'Required restaurant image columns are missing';
  END IF;

  IF to_regprocedure('public.restaurant_address_city_is_consistent(text,text)') IS NULL
     OR to_regprocedure('public.restaurant_source_is_publicly_displayable(uuid)') IS NULL
  THEN
    RAISE EXCEPTION 'Required restaurant public catalogue helpers are missing';
  END IF;
END;
$preflight$;
...
USING (
  ...
  AND (is_directory_listing IS FALSE OR directory_image_verified IS TRUE)
...
BEGIN
  ...
  WITH base_restaurants AS MATERIALIZED (
    ...
    WHERE r.is_active IS TRUE
      ...
      AND (r.is_directory_listing IS FALSE OR r.directory_image_verified IS TRUE)
      AND (
...
) IS 'Timeout-safe public restaurant catalogue: photo required; directory photos must be truth-verified.';
