-- Make validate_and_create_reservation tolerant to repeat bookings on the
-- same slot when the previous reservation already covers it (paid). The
-- failure mode being addressed: a user pays through Stripe twice for the
-- same slot (genuine UX confusion or browser retry). The second payment
-- now resolves to the existing reservation id instead of throwing
-- "Vous avez deja une reservation" — which prevented the function from
-- returning the reservation to the customer despite the money being
-- captured by Stripe.
--
-- Policy:
--   * If a non-cancelled reservation already exists at (user, restaurant,
--     date, time) and it is paid (any feature with total_amount > 0 or
--     metadata.paid='true'), return its id rather than throwing.
--   * Otherwise keep the original duplicate guard (pending / unpaid
--     duplicate clearly signals user error).
...
BEGIN
  ...
  IF v_feature_normalized = 'zero-attente' THEN
    ...
    -- Idempotent lookup by checkout_session_id (Stripe webhook retry, double trigger)
    SELECT id
  ...
  -- Duplicate slot check, with tolerance for paid duplicates: if the user
  -- already paid for the same slot, return that reservation id rather than
  -- throwing - the customer would otherwise be charged twice without a
  ...
  INSERT INTO public.reservations (
    ...
    preorder_items, metadata, notes, total_amount, payment_method,
    order_reference, updated_at
  )
  VALUES (
