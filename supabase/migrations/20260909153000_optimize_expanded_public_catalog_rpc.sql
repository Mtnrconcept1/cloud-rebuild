...
-- The public PostgREST role has statement_timeout=3s. After the verified
-- directory catalogue was expanded beyond image-backed rows, the previous RPC
-- repeatedly normalized and enriched every candidate before pagination and
-- could exceed that budget by an order of magnitude.
--
-- Keep the same public contract and visibility rules, but:
--   * use the existing restaurants.search_vector GIN index for text matching;
--   * aggregate categories and monthly/promotion metrics once per restaurant;
--   * avoid per-restaurant LATERAL scans;
--   * sort only the filtered candidate set and return 54 rows at most.
...
) IS 'Timeout-safe verified public catalogue pagination. Uses indexed full-text matching and grouped enrichment while keeping image-optional directory visibility.';
