-- Restaurateur account/billing: current subscription credits and detailed usage.
-- This RPC is used by the restaurant dashboard and keeps ownership checks server-side.
...
BEGIN
  ...
  WITH campaign_rows AS (
    SELECT
      ...
      GREATEST(COALESCE(ac.spent, 0), 0)::numeric(12, 2) AS credit_amount_chf,
    ...
    WHERE ac.restaurant_id = p_restaurant_id
      ...
      AND GREATEST(COALESCE(ac.spent, 0), 0) > 0
  )
  SELECT
    ...
    COALESCE(
      jsonb_agg(
        jsonb_build_object(
          ...
          'label', cr.title,
          'description', 'Campagne sponsorisee',
...
VALUES (
  'dashboard-billing',
  'Dashboard: Mon compte/Facturation',
  'Expose le compte restaurateur, l''abonnement, l''upgrade et le suivi des crédits.',
  true
)
