...
-- SECURITY DEFINER catalog searches bypass table RLS, so the real catalog must
-- reject every demo restaurant explicitly. The dedicated demo project receives
-- its own append-only migration and is not affected by this production filter.
...
COMMENT ON FUNCTION public.search_restaurants_catalog(
  ...
  'Real restaurant catalog. SECURITY DEFINER output excludes every is_demo row explicitly.';
