-- Commercial presentation AI is intentionally uncapped by daily request/cost
-- quotas. This migration is applied only to the dedicated TOK Commercial Demo
-- project through scripts/apply-commercial-demo-migrations.mjs.
--
-- Safety remains fail-closed: active session/account mapping, feature kill
-- switch, idempotency locks, stale-request recovery, provider circuit breaker,
-- global/session concurrency limits and request telemetry are preserved.
...
BEGIN
  ...
  -- Kept for observability/cost telemetry only. It is no longer a quota.
  ...
  -- Replays and new requests never bypass the server kill switch.
  IF NOT v_feature_enabled THEN
    RETURN jsonb_build_object('state', 'disabled', 'feature', 'commercial-demo-openai');
  END IF;
  ...
  -- No daily request-count or CHF budget limit is enforced in the dedicated
  -- presentation database. Provider attempt and cost columns remain populated
  -- so operations can still observe actual usage.
  IF NOT v_is_new THEN
    UPDATE public.commercial_demo_ai_requests request
...
COMMENT ON FUNCTION public.commercial_demo_ai_claim_request(
  uuid, uuid, uuid, uuid, uuid, text, text, text, uuid
) IS 'Claims isolated commercial demo AI requests without daily presentation quotas; feature kill switch, idempotency, concurrency and provider circuit breaker remain enforced.';
